...
from src.predict import predict_image  # ← NEW: body-part validator
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .disease-hint {
            ...
            margin-top: 0.5rem; min-height: 1.2em; transition: all 0.3s ease;
        }
        ...
        /* ── success check (green) ── */
        .result-title.success { color: var(--dark); }
        ...
        .error-banner .error-icon  { font-size: 3rem; margin-bottom: 1rem; }
        .error-banner .error-main  { font-size: 1.2rem; font-weight: 700; color: var(--danger); margin-bottom: 0.75rem; }
        .error-banner .error-sub   { font-size: 0.95rem; color: var(--gray); line-height: 1.6; }
        .error-banner .error-conf  { margin-top: 1rem; font-size: 0.85rem; color: var(--gray);
        ...
        <div class="main-content">
            ...
            <div class="right-panel">
                ...
                <div class="result-section">
                    ...
                    {% if validation_error %}
                    <!-- ── WRONG IMAGE TYPE ── -->
                    <div class="result-title error">Invalid Image</div>
                    <div class="error-banner">
                        <div class="error-icon">⚠️</div>
                        <div class="error-main">
                            Wrong image type uploaded!
                        </div>
                        <div class="error-sub">
                            You selected <strong>{{ expected_part | upper }}</strong> analysis
                            but the image appears to be a
                            <strong>{{ detected_part | upper }}</strong> scan.<br><br>
                            Please upload the correct
                            <strong>{{ expected_part | upper }}</strong> image and try again.
                        </div>
                    ...
                    {% else %}
                    <!-- ── NORMAL RESULTS ── -->
                    <div class="result-title success">Diagnosis Results</div>
...
# KL grade → clinical recommendation
...
# Dropdown value → expected body-part label (must match classes in predict.py)
EXPECTED_PART = {
    "lung":  "chest",
    "brain": "brain",
    "knee":  "knee",
...
def index():
    if request.method == "POST":
        ...
        if detected_part != expected_part:
            # Wrong image — show error, skip disease models entirely
